...
**Time intensity is an externally-anchored ruler that banded pairing cannot compress the way it compresses the logit scale.** That is the sharpest available story for this work sample.
...
**No Go-specific literature exists.** Every quantitative result here is transferred from chess or general psychometrics. The chess transfer is well-supported; the Go transfer is an assumption the work sample should state rather than assume.
...
**Validate the way Sackmann did, and put the table in the work sample.** On held-out attempts, report accuracy / Brier / log-loss for four estimators: overall-only, per-tag-only, fixed 50/50, and the σ_tag-derived adaptive weight. Reproducing the shape of Sackmann's clay row on Go Magic's own data is a stronger argument to a hiring manager than any citation.
...
**4. A concrete motivation for the rating clamp in `TODO.md`.** The guest describes a test account
...
clamps ratings to `[400, 4000]` and this repo does not — the open TODO now has a Go-specific
